...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    function getVoteData(){
        if (!rpcData.masternode_payments) return new Buffer.from([]);
    ...
    this.difficulty = parseFloat((0x00000000ffff0000000000000000000000000000000000000000000000000000 / this.target.toNumber()).toFixed(9));

    this.prevHashReversed = util.reverseByteOrder(new Buffer.from(rpcData.previousblockhash, 'hex')).toString('hex');
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce, versionMask){
        ...
        header.writeUInt32BE(versionMask, position + 32);
    ...
    this.registerSubmit = function(extraNonce1, extraNonce2, nTime, nonce, versionRollingBits){
        var submission = extraNonce1 + extraNonce2 + nTime + nonce + versionRollingBits;
    ...
    this.startSolution = function (coinbaseBuffer, merkleRoot, nTime, nonce, versionRollingBits) {
        console.info(JSON.stringify(this.rpcData))
        let version = (this.rpcData.version & ~parseInt('1fffe000', 16)) | versionRollingBits;
        console.info('' + this.rpcData.version + ' ' + versionRollingBits + ' ' + version)
        let headerBuffer = this.serializeHeader(merkleRoot, nTime, nonce, version);
    
        let finishSolution = function () {
            return {
              id: this.rpcData.id,
    ...
    this.getJobParams = function(){
        if (!this.jobParams){
            this.jobParams = [
                ...
                this.merkleBranch,
                util.packInt32BE(this.rpcData.version & ~parseInt('1fffe000', 16)).toString('hex'),
                this.rpcData.bits,
