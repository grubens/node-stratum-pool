...
var StratumClient = function(options){
    ...
    function handleConfigure(message){
        _this.emit("mining.configure", message);
        sendJson({
            id: message.id,
            result: {"version-rolling":true,"version-rolling.mask":"11110000"},
            error: null,
        });
    }
    ...
    function setupSocket(){
        ...
        socket.on('data', function(d){           
            dataBuffer += d;
            _this.emit("printSpecial", dataBuffer);
