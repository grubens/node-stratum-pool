...
exports.addressFromEx = function(exAddress, ripdm160Key){
    try {
        ...
        var addrBase = Buffer.concat([versionByte, new Buffer.from(ripdm160Key, 'hex')]);
...
exports.reverseBuffer = function(buff){
    var reversed = new Buffer.from(buff.length);
...
exports.varIntBuffer = function(n){
    if (n < 0xfd)
        ...
        var buff = new Buffer.from(3);
        buff[0] = 0xfd;
        buff.writeUInt16LE(n, 1);
        return buff;
    }
    else if (n <= 0xffffffff){
        var buff = new Buffer.from(5);
        buff[0] = 0xfe;
        buff.writeUInt32LE(n, 1);
        return buff;
    }
    else{
        var buff = new Buffer.from(9);
        buff[0] = 0xff;
...
exports.serializeNumber = function(n){
    ...
    if (n < 0xfd){
        var buff = new Buffer.from(2);
        buff[0] = 0x1;
        buff.writeUInt8(n, 1);
        return buff;
    }
    else if (n <= 0xffff){
        var buff = new Buffer.from(4);
        buff[0] = 0x3;
        buff.writeUInt16LE(n, 1);
        return buff;
    }
        ...
        return Buffer.concat([new Buffer.from([0x9]), binpack.packUInt64(n, 'little')]);
    }*/
...
exports.convertBitsToBuff = function(bitsBuff){
    ...
    var buff256 = new Buffer.from(32);
    buff256.fill(0);
